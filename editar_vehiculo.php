...
// Verificar permisos (roles permitidos: admin, mecanico, supervisor)
$rolesPermitidos = ['Admin', 'mecanico', 'supervisor'];
...
// Validar y obtener ID del vehículo
...
// Obtener datos actuales del vehículo con JOIN para cliente
$query = "SELECT v.*, c.nombre AS cliente_nombre, c.telefono AS cliente_telefono 
          FROM vehiculos v
...
// Tipos de vehículos y estados predefinidos
$tiposVehiculo = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($matricula) || strlen($matricula) < 4) {
        ...
        $checkMatricula = $conexion->prepare("SELECT id FROM vehiculos WHERE matricula = ? AND id != ?");
        $checkMatricula->bind_param('si', $matricula, $id_vehiculo);
    ...
    if ($kilometraje === false || $kilometraje < 0) {
        $kilometraje = null; // Permitir null si no es válido
    }

    // Procesar imagen si se subió
    $imagenNombre = $vehiculo['imagen'];
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, $extensionesPermitidas)) {
            ...
            if (!empty($imagenNombre) && file_exists('assets/img/vehiculos/' . $imagenNombre)) {
                unlink('assets/img/vehiculos/' . $imagenNombre);
            }
            
            // Generar nombre único para la nueva imagen
            $imagenNombre = 'veh_' . $id_vehiculo . '_' . time() . '.' . $extension;
            $rutaDestino = 'assets/img/vehiculos/' . $imagenNombre;
            
            if (!move_uploaded_file($_FILES['imagen']['tmp_name'], $rutaDestino)) {
                $errores[] = "Error al subir la imagen del vehículo";
                $imagenNombre = $vehiculo['imagen']; // Mantener la anterior si hay error
            }
        } else {
    ...
    if (empty($errores)) {
        try {
            ...
            $stmt = $conexion->prepare("UPDATE vehiculos SET 
                                      id_cliente = ?, marca = ?, modelo = ?, anio = ?, 
                                      matricula = ?, color = ?, kilometraje = ?, tipo = ?,
                                      estado = ?, observaciones = ?, imagen = ?
                                      WHERE id = ?");
            $stmt->bind_param("ississsssssi", 
                $id_cliente, $marca, $modelo, $anio, $matricula, $color,
                $kilometraje, $tipo, $estado, $observaciones, $imagenNombre, $id_vehiculo);
            
            if ($stmt->execute()) {
                // Registrar en el historial
                $accion = "Vehículo actualizado (Cambios: ";

                // Detectar cambios para el historial
                $camposCambiados = [];
                if ($vehiculo['id_cliente'] != $id_cliente) $camposCambiados[] = "cliente";
                if ($vehiculo['marca'] != $marca) $camposCambiados[] = "marca";
                if ($vehiculo['modelo'] != $modelo) $camposCambiados[] = "modelo";
                if ($vehiculo['anio'] != $anio) $camposCambiados[] = "año";
                if ($vehiculo['matricula'] != $matricula) $camposCambiados[] = "matrícula";
                if ($vehiculo['estado'] != $estado) $camposCambiados[] = "estado";
                if ($imagenNombre != $vehiculo['imagen']) $camposCambiados[] = "imagen";

                $accion .= implode(", ", $camposCambiados) . ")";
                ...
                throw new Exception("Error al actualizar el vehículo: " . $conexion->error);
            }
        } catch (Exception $e) {
            $conexion->rollback();
            
            // Eliminar imagen nueva si se subió pero falló el registro
        ...
        <div class="col-lg-10">
            <div class="card shadow-lg border-0 rounded-3">
                <div class="card-header bg-dark text-white py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h2 class="h5 mb-0 fw-bold">
                            <i class="fas fa-car me-2"></i> Editar Vehículo #<?= $vehiculo['id'] ?>
                        </h2>
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="editar_vehiculo.php?id=<?= $id_vehiculo ?>" enctype="multipart/form-data" class="needs-validation" novalidate>
                        <div class="row g-3">
                            <!-- Columna Izquierda -->
                            <div class="col-md-6">
                                        ...
                                        <select class="form-select" id="id_cliente" name="id_cliente" required>
                                            ...
                                            <?php while ($cliente = $clientes->fetch_assoc()): ?>
                                                <option value="<?= $cliente['id'] ?>" 
                                                    <?= (isset($_POST['id_cliente']) ? $_POST['id_cliente'] : $vehiculo['id_cliente']) == $cliente['id'] ? 'selected' : '' ?>>
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="marca" name="marca" 
                                                       value="<?= isset($_POST['marca']) ? htmlspecialchars($_POST['marca']) : htmlspecialchars($vehiculo['marca']) ?>" 
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="modelo" name="modelo" 
                                                       value="<?= isset($_POST['modelo']) ? htmlspecialchars($_POST['modelo']) : htmlspecialchars($vehiculo['modelo']) ?>" 
                                                       required minlength="2">
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="number" class="form-control" id="anio" name="anio" 
                                                       value="<?= isset($_POST['anio']) ? htmlspecialchars($_POST['anio']) : htmlspecialchars($vehiculo['anio']) ?>" 
                                                ...
                                                <select class="form-select" id="tipo" name="tipo">
                                                    ...
                                                    <?php foreach ($tiposVehiculo as $tipo): ?>
                                                        <option value="<?= htmlspecialchars($tipo) ?>" 
                                                            <?= (isset($_POST['tipo']) ? $_POST['tipo'] : ($vehiculo['tipo'] ?? '')) == $tipo ? 'selected' : '' ?>>
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control text-uppercase" id="matricula" name="matricula" 
                                                       value="<?= isset($_POST['matricula']) ? htmlspecialchars($_POST['matricula']) : htmlspecialchars($vehiculo['matricula']) ?>" 
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" id="color" name="color" 
                                                       value="<?= isset($_POST['color']) ? htmlspecialchars($_POST['color']) : htmlspecialchars($vehiculo['color']) ?>">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="number" class="form-control" id="kilometraje" name="kilometraje" 
                                                       value="<?= isset($_POST['kilometraje']) ? htmlspecialchars($_POST['kilometraje']) : htmlspecialchars($vehiculo['kilometraje']) ?>" 
                                                       min="0" step="1">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="estado" name="estado" required>
                                                    <?php foreach ($estadosVehiculo as $estadoOpt): ?>
                                                        <option value="<?= htmlspecialchars($estadoOpt) ?>" 
                                                            <?= (isset($_POST['estado']) ? $_POST['estado'] : $vehiculo['estado']) == $estadoOpt ? 'selected' : '' ?>>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <?php if (!empty($vehiculo['imagen'])): ?>
                                            <div class="mt-2">
                                                <img src="assets/img/vehiculos/<?= htmlspecialchars($vehiculo['imagen']) ?>" 
                                        ...
                                        <?php else: ?>
                                            <div class="mt-2 text-muted">No hay imagen actual</div>
                                        <?php endif; ?>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" id="observaciones" name="observaciones" 
                                                  rows="3"><?= isset($_POST['observaciones']) ? htmlspecialchars($_POST['observaciones']) : htmlspecialchars($vehiculo['observaciones'] ?? '') ?></textarea>
